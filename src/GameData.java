...
public class GameData {
	...
	private String tributeTheme;
	private String goreLevel;
	private String resurect;
	private boolean hasResurected;
	...
	public GameData(ArrayList<Tribute> users, ArrayList<Event> events,
			ArrayList<Event> dayOneEvents, int killNum,
			ArrayList<Event> nonLethal,  ArrayList<Item> items, int nonLethalNum) {
		this.users = users;
		this.events = events;
	...
	public void setResurect(String r) {
		resurect = r;
	}
	
	public String getResurect() {
		return resurect;
	}
	
	public int getMinLethal() {
		return minKill;
	}
	
	public void setMinLethal(int kill) {
		minKill = kill;
	}
	
	public int getMinNonLethal() {
		return minNonLethal;
	}
	
	public void setMinNonLethal(int nonLethal) {
		minNonLethal = nonLethal;
	}
	
	public void setLethal(ArrayList<Event> events) {
		this.events = events;
	}
	public void setNonLethal(ArrayList<Event> events) {
		nonLethal = events;
	}
	public void setHasResurected(boolean b) {
		hasResurected = b;
	}
	public boolean getHasResurected() {
		return hasResurected;
	}
	...
	public ArrayList<String> nextDay() {
		...
		for (int i = 0; i < toKill; i++) { // Select a death tribute for each
			...
			boolean flag = false;
			if(hasResurected == false && !resurect.equals("None")) {
				if(checkResurect(lastUser, resurect)){
					flag = true;
				}
				else {
			...
			if(flag == true){
				dead.add(lastUser.getName() + " has been resurected!");
				hasResurected = true;
			}
			if (users.size() == 1) {
	...
	public ArrayList<String> getFemaleNames() {
		...
		if (tributeTheme.equals("Hunger Games")) {
			// Hunger Games Theme
			names.add("Glimmer");
			names.add("Clove");
			names.add("Cashmere");
			...
			names.add("Rue");
			names.add("Annie");
		} else if (tributeTheme.equals("Harry Potter")) {
			// Harry Potter Theme
		...
		} else if (tributeTheme.equals("Family Guy")) {
			// Family Guy Theme
		...
		} else if (tributeTheme.equals("The Simpsons")) {
			// The Simpsons Theme
	...
	/**
	 * checks if tribute can be resurrected
	 * @param tribute
	 * @param res
	 * @return
	 */
	private boolean checkResurect(Tribute tribute, String res) {
		int district = Integer.parseInt(res.substring(9, 10));
		boolean isMale = false;
		if(res.substring(11).equals("Male")) {
			isMale = true;
		}
		if(tribute.getDistrict() == district && tribute.getMale() == isMale) return true;
		else return false;
	}
